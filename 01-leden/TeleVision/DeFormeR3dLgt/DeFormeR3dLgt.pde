...
void setup() {
  ...
  //---------------------> video
  m = new JMyron();//obj
  m.start(320,240);//320x240//360,288//352,288
  m.findGlobs(0);
  //smooth();
  camera1 = new Camera(this, width/2, height/2, -250,width/2, height/2, 0);
...
void vidko() {
  ...
  for(int y=0;y<m.height();y+=2){ 
    ...
    for(int x=0;x<m.width();x+=3){ 
      ...
      stroke(avgr,int(Y[y*m.width()+x]+30)*8);
      noFill();
      
      vertex(x+160,y+120,(Y[y*m.width()+x]));
    }
    endShape();
  }
...
stroke(255);
      //rect(-5,-5,m.width()+10,m.height()+10);
  //hlavicka();
