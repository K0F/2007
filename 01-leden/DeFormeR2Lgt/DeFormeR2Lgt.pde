...
void setup() {
  ...
  //-----------------------* frame title
  frame.setTitle("Woody Vasulka's wave poem");
  //-----------------------*
...
void vidko() {
  ...
  for(int y=0;y<m.height();y+=2){ 
    ...
    for(int x=0;x<m.width();x+=2){ 
      av = (red(img[y*m.width()+x])+green(img[y*m.width()+x])+blue(img[y*m.width()+x]))/3.0;
      ...
      //rect(x,y,4,4);
      vertex(map(x,0,m.width(),0,width),map((y+(Y[y*m.width()+x])),0,m.height(),0,height));
    }
    endShape();
  }
