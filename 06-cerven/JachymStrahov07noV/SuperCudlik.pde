class SuperCudlik{
  ...
  void ctvrty(int _kk){ // rotopanoramatikon
    ...
    rotonRot+=(rotonInput-rotonRot)/3.0;
     ...
     if ( rotonFrame != null ) {image( rotonFrame, 100,0);}
     
    if(rozbaleno){
      
      line(map(rotonRot,0.0,2.0,0.0,700.0),0,map(rotonRot,0.0,2.0,0.0,700.0),10);
    if(abs(roton.time()-rotonRot)>0.04){roton.jump(rotonRot);}
      } 
