...
MovieMaker movie;
boolean accept = true;
 float X = -720, X2 = -720,POSUVNIK = -720;
 ...
 void setup(){
  ...
  frameRate(25);
   ...
   datos=loadStrings(sketchPath+"/machinaL_Start.pde");
   movie = new MovieMaker(this, width, height, "vids/start.mov",MovieMaker.JPEG,MovieMaker.HIGH,25) ;
 }
 ...
 void draw(){
  ...
  fill(c[0],map(POSUVNIK,-720,0,0,85));
  noStroke();
...
class Loader{
 ...
 void rotatko(){
   ...
   translate(490,-40);
  pushMatrix();
  //theta+=0;//(osc.getValue()-theta)/100.0f;
