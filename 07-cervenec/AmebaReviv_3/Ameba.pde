 ...
 float distance;
  ...
  //    if(distance<=((R+a[i].R))){
    ...
    void collision(int _idC,int _dis){
     for(int i = 0;i<cl.length;i++){
       for(int q = 0;q<a[_idC].cl.length;q++){
      if((abs(cl[i].x-a[_idC].cl[q].x)<_dis)&&(abs(cl[i].y-a[_idC].cl[q].y)<_dis)){
       cl[i].x+=(cl[i].x-a[_idC].cl[q].x)/(0.5*cl[i].stredDist);
       cl[i].y+=(cl[i].y-a[_idC].cl[q].y)/(0.5*cl[i].stredDist);
      } 
