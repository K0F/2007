...
void dropEvent(DropEvent theDropEvent) {
  if(theDropEvent.isFile()) {
    ...
    center_X=0;center_Y=0;size=1;speed=40;
  }
...
void sw(int x){
  if(cnt[x]>6){playable[x]=1;cnt[x]=0;}
  if((playable[x]==1)&&(!mute)){
  Note note;
  note = new Note((x+10)*2,80,100);
  //swSet((int)(map(constrain(x,1,20),1,30,1,25)),0,0);
