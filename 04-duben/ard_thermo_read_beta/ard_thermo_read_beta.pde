...
//println((prum*12.0));
  noSmooth();
  //----------------------------------- * hodina
  if((changeH[0] != changeH[1])&&(!first)) {stroke(255,20);fill(255,180);line(cnt,0,cnt,height);text(changeH[0],cnt+5,height-2);}
  changeH[1] = (int)hour();
  
 //------------------------------------ * den
  if((changeD[0] != changeD[1])&&(!first)) {savePicture();background(0);}
  changeD[1] = (int)day();
...
void savePicture(){
 ...
 save(sketchPath+"/imgs/light_"+(int)day()+"-"+(int)month()+"_"+(int)year()+"_"+f+".png");
